...
final class HPKETest: XCTestCase {
    ...
    func test1() throws {
        for test in testCases1 {
            ...
            let hpkeS = try Sender(test.ikm, suite, PublicKey(kem: test.kem, bytes: test.pkR), info)
            ...
            XCTAssertEqual(try hpkeS.sendExport(context: [], L: 32), test.exp1)
            XCTAssertEqual(try hpkeS.sendExport(context: [0], L: 32), test.exp2)
            XCTAssertEqual(try hpkeS.sendExport(context: expCtx, L: 32), test.exp3)
        }
        for test in testCases2 {
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(test.ikm, suite, PublicKey(kem: test.kem, bytes: test.pkR), info, psk, psk_id)
            XCTAssertEqual(test.enc, hpkeS.encapsulatedKey)
            XCTAssertEqual(test.ct0, try hpkeS.seal(pt: pt, aad: aad0))
            XCTAssertEqual(test.ct1, try hpkeS.seal(pt: pt, aad: aad1))
            XCTAssertEqual(test.ct2, try hpkeS.seal(pt: pt, aad: aad2))
        ...
        for test in testCases3 {
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(test.ikm, suite, PublicKey(kem: test.kem, bytes: test.pkR), info, PrivateKey(kem: test.kem, bytes: test.skS))
            XCTAssertEqual(test.enc, hpkeS.encapsulatedKey)
            XCTAssertEqual(test.ct0, try hpkeS.seal(pt: pt, aad: aad0))
            XCTAssertEqual(test.ct1, try hpkeS.seal(pt: pt, aad: aad1))
            XCTAssertEqual(test.ct2, try hpkeS.seal(pt: pt, aad: aad2))
        ...
        for test in testCases4 {
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(test.ikm, suite, PublicKey(kem: test.kem, bytes: test.pkR), info, PrivateKey(kem: test.kem, bytes: test.skS), psk, psk_id)
            XCTAssertEqual(test.enc, hpkeS.encapsulatedKey)
            XCTAssertEqual(test.ct0, try hpkeS.seal(pt: pt, aad: aad0))
            XCTAssertEqual(test.ct1, try hpkeS.seal(pt: pt, aad: aad1))
            XCTAssertEqual(test.ct2, try hpkeS.seal(pt: pt, aad: aad2))
    ...
    func test2() throws {
        ...
        for test in testCases1 {
            ...
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(suite: suite, publicKey: PublicKey(kem: test.kem, bytes: test.pkR), info: info)
            for i in 0 ..< n {
                ct[i] = try hpkeS.seal(pt: pt, aad: [Byte(i)])
            }
            let hpkeR = try Recipient(suite: suite, privateKey: PrivateKey(kem: test.kem, bytes: test.skR), info: info, encap: hpkeS.encapsulatedKey)
            for i in 0 ..< n {
        ...
        for test in testCases2 {
            var ct: [Bytes] = [Bytes](repeating: [], count: n)
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(suite: suite, publicKey: PublicKey(kem: test.kem, bytes: test.pkR), info: info, psk: psk, pskId: psk_id)
            for i in 0 ..< n {
                ct[i] = try hpkeS.seal(pt: pt, aad: [Byte(i)])
            }
            let hpkeR = try Recipient(suite: suite, privateKey: PrivateKey(kem: test.kem, bytes: test.skR), info: info, psk: psk, pskId: psk_id, encap: hpkeS.encapsulatedKey)
            for i in 0 ..< n {
                let ptx = try hpkeR.open(ct: ct[i], aad: [Byte(i)])
                XCTAssertEqual(ptx, pt)
            }
        ...
        for test in testCases3 {
            var ct: [Bytes] = [Bytes](repeating: [], count: n)
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(suite: suite, publicKey: PublicKey(kem: test.kem, bytes: test.pkR), info: info, authentication: PrivateKey(kem: test.kem, bytes: test.skS))
            for i in 0 ..< n {
                ct[i] = try hpkeS.seal(pt: pt, aad: [Byte(i)])
            }
            let hpkeR = try Recipient(suite: suite, privateKey: PrivateKey(kem: test.kem, bytes: test.skR), info: info, authentication: PublicKey(kem: test.kem, bytes: test.pkS), encap: hpkeS.encapsulatedKey)
            for i in 0 ..< n {
                let ptx = try hpkeR.open(ct: ct[i], aad: [Byte(i)])
                XCTAssertEqual(ptx, pt)
            }
        ...
        for test in testCases4 {
            var ct: [Bytes] = [Bytes](repeating: [], count: n)
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let hpkeS = try Sender(suite: suite, publicKey: PublicKey(kem: test.kem, bytes: test.pkR), info: info, authentication: PrivateKey(kem: test.kem, bytes: test.skS), psk: psk, pskId: psk_id)
            for i in 0 ..< n {
                ct[i] = try hpkeS.seal(pt: pt, aad: [Byte(i)])
            }
            let hpkeR = try Recipient(suite: suite, privateKey: PrivateKey(kem: test.kem, bytes: test.skR), info: info, authentication: PublicKey(kem: test.kem, bytes: test.pkS), psk: psk, pskId: psk_id, encap: hpkeS.encapsulatedKey)
            for i in 0 ..< n {
                let ptx = try hpkeR.open(ct: ct[i], aad: [Byte(i)])
                XCTAssertEqual(ptx, pt)
            }
    ...
    func testSingleShot() throws {
        for test in testCases1 {
            let suite = CipherSuite(kem: test.kem, kdf: test.kdf, aead: test.aead)
            let pubKey = try PublicKey(kem: test.kem, bytes: test.pkR)
            let privKey = try PrivateKey(kem: test.kem, bytes: test.skR)
            for i in 0 ..< 3 {
        ...
        for test in testCases3 {
            ...
            let pubKey = try PublicKey(kem: test.kem, bytes: test.pkR)
            let privKey = try PrivateKey(kem: test.kem, bytes: test.skR)
            let authPubKey = try PublicKey(kem: test.kem, bytes: test.pkS)
            let authPrivKey = try PrivateKey(kem: test.kem, bytes: test.skS)
            for i in 0 ..< 3 {
            ...
            let (encap, secret) = try suite.sendExport(publicKey: pubKey, info: info, context: expCtx, L: 10, authentication: authPrivKey)
            let secret1 = try suite.receiveExport(privateKey: privKey, info: info, context: expCtx, L: 10, authentication: authPubKey, encap: encap)
        ...
        for test in testCases4 {
            ...
            let (encap, secret) = try suite.sendExport(publicKey: pubKey, info: info, context: expCtx, L: 10, authentication: authPrivKey, psk: psk, pskId: psk_id)
            let secret1 = try suite.receiveExport(privateKey: privKey, info: info, context: expCtx, L: 10, authentication: authPubKey, psk: psk, pskId: psk_id, encap: encap)
            XCTAssertEqual(secret, secret1)
        }
    }
    ...
    func test384() throws {
        ...
        let hpkeS1 = try Sender(suite: suite1, publicKey: PublicKey(kem: .P384, bytes: pkR384), info: info)
        for i in 0 ..< n {
            ct[i] = try hpkeS1.seal(pt: pt, aad: [Byte(i)])
        }
        let hpkeR1 = try Recipient(suite: suite1, privateKey: PrivateKey(kem: .P384, bytes: skR384), info: info, encap: hpkeS1.encapsulatedKey)
        for i in 0 ..< n {
            let ptx = try hpkeR1.open(ct: ct[i], aad: [Byte(i)])
            XCTAssertEqual(ptx, pt)
        }
        let suite2 = CipherSuite(kem: .P384, kdf: .KDF256, aead: .CHACHAPOLY)
        let hpkeS2 = try Sender(suite: suite2, publicKey: PublicKey(kem: .P384, bytes: pkR384), info: info)
        for i in 0 ..< n {
            ct[i] = try hpkeS2.seal(pt: pt, aad: [Byte(i)])
        }
        let hpkeR2 = try Recipient(suite: suite2, privateKey: PrivateKey(kem: .P384, bytes: skR384), info: info, encap: hpkeS2.encapsulatedKey)
        for i in 0 ..< n {
        ...
        // single-shot test
        for i in 0 ..< 3 {
            let (enc, ct) = try suite1.seal(publicKey: PublicKey(kem: .P384, bytes: pkR384), info: info, pt: pt, aad: [Byte(i)])
            let ptx = try suite1.open(privateKey: PrivateKey(kem: .P384, bytes: skR384), info: info, ct: ct, aad: [Byte(i)], encap: enc)
            XCTAssertEqual(ptx, pt)
        }
        for i in 0 ..< 3 {
            let (enc, ct) = try suite2.seal(publicKey: PublicKey(kem: .P384, bytes: pkR384), info: info, pt: pt, aad: [Byte(i)])
            let ptx = try suite2.open(privateKey: PrivateKey(kem: .P384, bytes: skR384), info: info, ct: ct, aad: [Byte(i)], encap: enc)
            XCTAssertEqual(ptx, pt)
        }
    ...
    func test448() throws {
        ...
        let hpkeS1 = try Sender(suite: suite1, publicKey: PublicKey(kem: .X448, bytes: pkR448), info: info)
        for i in 0 ..< n {
            ct[i] = try hpkeS1.seal(pt: pt, aad: [Byte(i)])
        }
        let hpkeR1 = try Recipient(suite: suite1, privateKey: PrivateKey(kem: .X448, bytes: skR448), info: info, encap: hpkeS1.encapsulatedKey)
        for i in 0 ..< n {
            let ptx = try hpkeR1.open(ct: ct[i], aad: [Byte(i)])
            XCTAssertEqual(ptx, pt)
        }
        let suite2 = CipherSuite(kem: .X448, kdf: .KDF256, aead: .CHACHAPOLY)
        let hpkeS2 = try Sender(suite: suite2, publicKey: PublicKey(kem: .X448, bytes: pkR448), info: info)
        for i in 0 ..< n {
            ct[i] = try hpkeS2.seal(pt: pt, aad: [Byte(i)])
        }
        let hpkeR2 = try Recipient(suite: suite2, privateKey: PrivateKey(kem: .X448, bytes: skR448), info: info, encap: hpkeS2.encapsulatedKey)
        for i in 0 ..< n {
            let ptx = try hpkeR2.open(ct: ct[i], aad: [Byte(i)])
            XCTAssertEqual(ptx, pt)
        }
        ...
        // single-shot test
        for i in 0 ..< 3 {
            let (enc, ct) = try suite1.seal(publicKey: PublicKey(kem: .X448, bytes: pkR448), info: info, pt: pt, aad: [Byte(i)])
            let ptx = try suite1.open(privateKey: PrivateKey(kem: .X448, bytes: skR448), info: info, ct: ct, aad: [Byte(i)], encap: enc)
            XCTAssertEqual(ptx, pt)
        }
        for i in 0 ..< 3 {
            let (enc, ct) = try suite2.seal(publicKey: PublicKey(kem: .X448, bytes: pkR448), info: info, pt: pt, aad: [Byte(i)])
            let ptx = try suite2.open(privateKey: PrivateKey(kem: .X448, bytes: skR448), info: info, ct: ct, aad: [Byte(i)], encap: enc)
            XCTAssertEqual(ptx, pt)
        }
    }
