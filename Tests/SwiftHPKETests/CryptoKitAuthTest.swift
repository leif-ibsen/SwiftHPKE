...
final class CryptoKitAuthTest: XCTestCase {
    ...
    func doTest25519_1(_ info: Bytes, _ auth: Bytes) throws {
        ...
        var ckSender = try CryptoKit.HPKE.Sender(recipientKey: ckPub, ciphersuite: ckSuite, info: Data(info), authenticatedBy: ckAuth)
        
        let ct = try ckSender.seal(Data(msg.utf8), authenticating: auth)
        let hpRecipient = try Recipient(suite: hpSuite, privateKey: hpPriv, info: info, authentication: hpAuthPub, encap: Bytes(ckSender.encapsulatedKey))
    ...
    func doTest25519_2(_ info: Bytes, _ auth: Bytes) throws {
        ...
        let hpSender = try Sender(suite: hpSuite, publicKey: hpPub, info: info, authentication: hpAuthPriv)

        let ct = try hpSender.seal(pt: Bytes(msg.utf8), aad: auth)
        var ckRecipient = try CryptoKit.HPKE.Recipient(privateKey: ckPriv, ciphersuite: ckSuite, info: Data(info), encapsulatedKey: Data(hpSender.encapsulatedKey), authenticatedBy: ckAuthPub)
