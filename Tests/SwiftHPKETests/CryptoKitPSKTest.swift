...
final class CryptoKitPSKTest: XCTestCase {
    ...
    func doTest25519_1(_ info: Bytes, _ psk: Bytes, _ pskId: Bytes, _ auth: Bytes) throws {
        ...
        var ckSender = try CryptoKit.HPKE.Sender(recipientKey: ckPub, ciphersuite: ckSuite, info: Data(info), presharedKey: SymmetricKey(data: psk), presharedKeyIdentifier: Data(pskId))
        
        let ct = try ckSender.seal(Data(msg.utf8), authenticating: auth)
        let hpRecipient = try Recipient(suite: hpSuite, privateKey: hpPriv, info: info, psk: psk, pskId: pskId, encap: Bytes(ckSender.encapsulatedKey))
    ...
    func doTest25519_2(_ info: Bytes, _ psk: Bytes, _ pskId: Bytes, _ auth: Bytes) throws {
        ...
        let hpSender = try Sender(suite: hpSuite, publicKey: hpPub, info: info, psk: psk, pskId: pskId)
        
        let ct = try hpSender.seal(pt: Bytes(msg.utf8), aad: auth)
        var ckRecipient = try CryptoKit.HPKE.Recipient(privateKey: ckPriv, ciphersuite: ckSuite, info: Data(info), encapsulatedKey: Data(hpSender.encapsulatedKey), presharedKey: SymmetricKey(data: psk), presharedKeyIdentifier: Data(pskId))
